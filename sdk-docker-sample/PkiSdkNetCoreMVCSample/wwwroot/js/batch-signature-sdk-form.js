...
// ----------------------------------------------------------------------------------------------------------
var batchSignatureSdkForm = (function () {
	...
	function onBatchCompleted() {
		...
		addAlert('primary', 'Each document tooks approximately ' + ((endTime - startTime) / formElements.documentsIds.length).toFixed(2) + ' milliseconds.');
		addAlert('primary', 'It signs approximately ' + ((formElements.documentsIds.length * 1000)/(endTime - startTime)).toFixed(2) + ' documents for seconds.');
	...
	function renderSuccess(step) {
		...
		docLi
			...
			.append($('<a>')
				...
				.attr('href', '/Download/File/' + step.signedFileId));
	}
